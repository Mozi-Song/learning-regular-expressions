...
### 8.如何开启多行模式，它适用于什么场景？
把`(?m)`放在开头可开启多行模式。
多行模式迫使正则表达式引擎将换行符视为字符串分隔符，这样`^`既可以匹配字符串开头，也可匹配换行符之后的起始位置（新行）；`$`既能匹配字符串结尾也可匹配换行符之后的结束位置。
它适用于需要匹配
多行模式破事正则表达式引擎将换行符视为字符串分隔符，这样`^`既可以匹配字符串开头，也可匹配换行符之后的起始位置（新行）；`$`既能匹配字符串结尾也可匹配换行符之后的结束位置。匹配匹配
多行模式破事正则表达式引擎将换行符视为字符串分隔符，这样`^`既可以匹配字符串开头，也可匹配换行符之后的起始位置（新行）；`$`既能匹配字符串结尾也可匹配换行符之后的结束位置。匹配
